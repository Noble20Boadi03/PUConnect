...
const ChatPage: React.FC = () => {
      ...
      try {
        ...
        rawMessages.forEach((msg: any) => {
          ...
          if (!convMap.has(key)) {
            ...
            if (
              new Date(msg.created_at) > new Date(existing.last_message_time)
            ) {
  ...
  useEffect(() => {
    if (partnerId) {
      const currentConv = conversations.find(
        (c) => c.user_id === partnerId && c.listing_id === listingId,
      );
      if (currentConv) {
  ...
  if (!user)
    return (
      <div className="p-4 text-center mt-20">Please log in to use chat.</div>
    );
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] bg-white overflow-hidden">
      {/* Sidebar */}
      <div
        className={`w-full md:w-80 lg:w-96 border-r flex flex-col shrink-0 transition-all ${partnerId ? "hidden md:flex" : "flex"}`}
      >
      ...
      {/* Chat Area */}
      <div
        className={`flex-1 flex flex-col min-w-0 bg-gray-50 transition-all ${!partnerId ? "hidden md:flex" : "flex"}`}
      >
        {partnerId ? (
