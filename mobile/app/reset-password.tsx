...
import { useAppRouter, useThemeColor, useAuth } from '../hooks';
...
export default function ResetPasswordScreen() {
  ...
  const targetEmailOrUsername = emailOrUsername || user?.email || user?.username || '';
  ...
  const handleResetPassword = async () => {
    ...
    try {
      const response = await authService.resetPassword(targetEmailOrUsername, otp, newPassword, confirmPassword);
      setSuccessMsg(response.message ? `${response.message} You will remain logged in.` : 'Password reset successfully! You will remain logged in.');
  ...
  const handleResendOTP = async () => {
    if (!targetEmailOrUsername) return;
    ...
    try {
      const response = await authService.forgotPassword(targetEmailOrUsername);
      setSuccessMsg(response.message ?? 'OTP has been resent to your email.');
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
    } catch (error: any) {
          ...
          <View style={[styles.card, { backgroundColor: cardBg }]}>
            <View style={styles.titleContainer}>
              ...
              <Text style={[styles.subtitle, { color: Colors.text }]}>
                Enter the OTP sent to your email and create a new password. You will stay signed in on this device.
              </Text>
            </View>
